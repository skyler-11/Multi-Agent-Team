...
The design language opens with a clean, high-contrast digital interface framework[cite: 1]. A wide horizontal band anchoring deep, structural brand tones occupies the upper third of nearly every marketing page — the platform's instantly-recognizable atmospheric backdrop[cite: 1]. Type and product UI mockups float above it on `{colors.canvas}` (light mode gray-white), with the structural frame acting as both decoration and visual anchor[cite: 1]. The lower portion of the page returns to white, with feature explanations on `{colors.canvas-soft}` (a soft cool-grey with a purple tint) and dashboard product mockups composited as faux IDE/console panels in deep anchor purple[cite: 1].

The color system has two primary roles. **Orange 400** (`{colors.primary}` — `#FF4208`) is the brand's high-voltage primary CTA color, used sparingly to drive maximum conversion pop: one filled pill per band[cite: 1]. **Deep anchor purple** (`{colors.ink}` — `#1E0F45`) is the universal headline anchor and the fill of dashboard mockups, providing immense structural weight[cite: 1]. Secondary purple tones appear inside interactive layers as secondary buttons, badges, and filters; they are never allowed to directly overlap high-saturation orange to eliminate visual vibration[cite: 1].
...
**Key Characteristics:**
- High-contrast structural backdrop on every marketing hero — deep anchor purple washed across the upper third of the page to organize the canvas[cite: 1].
- Single-orange CTA hierarchy: filled `{colors.primary}` pill is the only filled button on marketing surfaces[cite: 1].
...
- Dark-app dashboard track: deep anchor purple product UI mockups sit composited above the white canvas, frequently with rendered code or dashboard tables inside[cite: 1].
- Pill-shaped buttons (`{rounded.pill}` 9999px) with tight `8px 16px` padding — short, decisive, transactional.
- Soft purple-tint feature cards (`{colors.canvas-cream}`) introduce a balanced structural interlude without causing visual fatigue[cite: 1].
...
### Brand & Accent
- **Orange 400** (`{colors.primary}` — `#FF4208`): The brand's high-voltage primary CTA accent color[cite: 1]. Filled-pill button, high-conversion focal point[cite: 1].
- **Orange 500** (`{colors.primary-deep}` — `#DA3806`): A deeper orange used as the press-state and hover-state warmer alternative[cite: 1].
- **Purple 500** (`{colors.primary-press}` — `#341C7D`): Pressed-state background adjustment for secondary elements[cite: 1].
- **Orange 300** (`{colors.primary-soft}` — `#FF693B`): A mid-tone orange accent used for alert borders and high-visibility graph indicators[cite: 1].
- **Orange 100** (`{colors.primary-bg-subdued-hover}` — `#FFB7A1`): Pale orange fill used strictly at low opacity for soft tag backgrounds[cite: 1].
- **Purple 600** (`{colors.brand-dark-900}` — `#1E0F45`): The deep structural anchor purple used on the featured pricing tier, headers, and dashboard chrome[cite: 1].
- **Purple 400** (`{colors.ruby}` — `#4827AF`): Secondary interactive color; links, icons, active filters, and secondary button boundaries[cite: 1].
- **Purple 300** (`{colors.magenta}` — `#5C37D2`): Mid-scale purple for nested dashboard accents and interactive highlights[cite: 1].
- **Purple 200** (`{colors.lemon}` — `#7E60DC`): A lighter purple layer utilized inside complex code or data visualizations[cite: 1].

### Surface
- **Canvas** (`{colors.canvas}` — `#FFFFFF`): Default page background light mode canvas[cite: 1].
- **Canvas Soft** (`{colors.canvas-soft}` — `#F8F7FC`): Soft cool-grey canvas background with a purple tint to give a clean surface[cite: 1].
- **Canvas Dark** (`{colors.canvas-cream}` — `#0B0813`): Deep midnight canvas background used for the dark mode theme[cite: 1].
- **Hairline** (`{colors.hairline}` — `#E5E0F2`): 1px borders on cards and tables to maintain a soft purple undertone[cite: 1].
- **Hairline Input** (`{colors.hairline-input}` — `#27213D`): Darker, crisp hairline used for high-contrast form inputs and dividers[cite: 1].

### Text
- **Ink** (`{colors.ink}` — `#0B0813`): Default primary body text color across the brand[cite: 1]. Deep midnight-black with purple temperature[cite: 1].
- **Ink Secondary** (`{colors.ink-secondary}` — `#151124`): Elevated dark surface neutral and heavy subheading text[cite: 1].
- **Ink Mute** (`{colors.ink-mute}` — `#6E6582`): Muted text, helper text, captions, table labels[cite: 1].
- **Ink Mute 2** (`{colors.ink-mute-2}` — `#6E6582`): Consistent secondary helper text used in navigation frameworks[cite: 1].
- **On Primary** (`{colors.on-primary}` — `#FFFFFF`): Accessible white text on orange, deep purple, or dark-navy surfaces[cite: 1].
...
- Background `{colors.primary}`, text `{colors.on-primary}`, type `{typography.button-md}`, padding `{spacing.sm} {spacing.lg}` (8px 16px), rounded `{rounded.pill}` 9999px[cite: 1].
- Pressed state `button-primary-pill-pressed` shifts background to `{colors.primary-deep}`[cite: 1].

**`button-secondary`** — outline-style alternative.
- Background `{colors.canvas}`, text `{colors.ruby}`, 1.5px solid `{colors.ruby}` border, same pill geometry[cite: 1].

**`button-on-dark`** — used on dashboard / dark surfaces.
- Background `{colors.brand-dark-900}`, text `{colors.on-primary}`, same pill geometry[cite: 1].

### Cards & Containers

**`card-feature-light`** — feature explanation card on white.
- Background `{colors.canvas}`, padding `{spacing.xxl}`, rounded `{rounded.lg}` 12px, 1px `{colors.hairline}` border, optional Level 1 shadow[cite: 1].

**`card-pricing`** — standard pricing tier.
- Background `{colors.canvas}`, padding `{spacing.xxl}`, rounded `{rounded.lg}`, 1px `{colors.hairline}` border[cite: 1]. Title `{typography.heading-lg}`, price `{typography.display-md}`, body `{typography.body-md}`, CTA pinned bottom as `button-primary-pill`[cite: 1].

**`card-pricing-featured`** — the inverted dark featured tier.
- Background `{colors.brand-dark-900}`, text `{colors.on-primary}`, otherwise identical structure to `card-pricing`[cite: 1]. The deep-purple fill is the brand's distinctive featured-tier choice[cite: 1].

**`card-cream-band`** — warm interlude card.
- Background `{colors.canvas-cream}`, text `{colors.ink}`, padding `{spacing.xxl}`, rounded `{rounded.lg}`[cite: 1]. Used to break up the orange / white rhythm with a structural dark canvas variant[cite: 1].

**`card-dashboard-mockup`** — composited dashboard / product UI screenshot.
- Background `{colors.canvas}`, type `{typography.body-tabular}` (with `tnum`), padding `{spacing.xl}` 24px, rounded `{rounded.lg}` 12px, Level 2 shadow[cite: 1]. Often contains nested mini-mockups: code preview + dashboard table + chart card.

### Inputs & Forms

**`text-input`** — standard form field.
- Background `{colors.canvas}`, text `{colors.ink}`, type `{typography.body-md}`, padding `{spacing.sm} {spacing.md}` (8px 12px), rounded `{rounded.sm}` 6px, 1px `{colors.hairline-input}` border[cite: 1].
- Focus state `text-input-focused`: border swaps to `{colors.primary}`[cite: 1].

### Navigation

**`nav-bar-on-mesh`** — top nav floating over the gradient hero.
- Background `{colors.canvas}` (or transparent depending on scroll), text `{colors.ink}`, padding `{spacing.lg} {spacing.xl}`[cite: 1]. Logo wordmark on the left, primary nav center, sign-in + filled `button-primary-pill` on the right[cite: 1].

### Pills, Tags, and Chips

**`pill-tag-soft`** — subdued orange tag.
- Background `{colors.primary-bg-subdued-hover}`, text `{colors.primary-deep}`, type `{typography.micro-cap}`, padding `4px 8px`, rounded `{rounded.pill}`[cite: 1].

### Signature Components

**Gradient Mesh Backdrop** — structural anchor frames and clean vector compositions layered with plenty of negative space to break up structural panels safely[cite: 1].
...
- Text `{colors.ruby}` rendered in `{typography.body-md}`, no underline by default[cite: 1].

**`footer-light`** — site-wide footer.
- Background `{colors.canvas}`, text `{colors.ink-mute}`, type `{typography.caption}`, padding `{spacing.huge} {spacing.xl}` (64px 24px)[cite: 1]. Holds 4–6 columns of link groups, social icons, and a small legal row.

## Do's and Don'ts

### Do
- Reserve `{colors.primary}` for filled CTAs and inline action emphasis — it should appear sparingly, one filled button per band[cite: 1].
- Apply the dark structural backdrop frameworks to every marketing hero to guide layout mapping[cite: 1].
...
- Don't add new accent colors outside the documented high-contrast platform scales[cite: 1].
- Don't use the high-saturation orange `{colors.primary}` directly over secondary purple text elements — it violates WCAG rules and triggers optical vibration[cite: 1].
...
7. The layout structure hierarchy is non-negotiable on marketing heroes — bare-canvas heroes break the brand[cite: 1].